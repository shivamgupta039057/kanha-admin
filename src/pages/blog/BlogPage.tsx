...
import { API_BANQUET_DELETE, API_BANQUET_GET, API_DELETE_BLOG, API_DELETE_GALLERY, API_GET_BLOG, API_GET_GALLERY, API_ROOM_DELETE, API_ROOM_GET } from '../../utils/APIConstant';
import { useSelector } from 'react-redux';
import AddRoomsBooking from './AddRoomBooking';
import AddBlogPageModal from './AddBlogPageModal';
...
console.log("openModalopenModalopenModal" , openModal);
  ...
  const { data: roomData } = useQuery({
    ...
    queryFn: () =>
      Apiservice.getAuth(`${API_GET_BLOG}`, token ?? ""),
    staleTime: 5 * 60 * 1000,
  });
  ...
  const deleteRoomMutation = useMutation({
    mutationFn: async (id: string) => {
      return await Apiservice.postAuth(`${API_DELETE_BLOG}/${id}`, {}, token);
    },
    onSuccess: (res: any) => {
  ...
  const columns: MRT_ColumnDef<BlogData>[] = [
    {
      header: 'ID',
      accessorKey: 'SrNo',
      enableSorting: false,
    ...
    },
    {
      ...
      Cell: ({ cell }) => {
        ...
        // Optionally strip HTML tags for preview, or show as is
        const plainText = value
          ? value.replace(/<[^>]+>/g, '').slice(0, 80) + (value.length > 80 ? '...' : '')
          : 'N/A';
        return plainText;
      },
    },
