...
import { API_ROOM_DELETE, API_ROOM_GET } from '../../utils/APIConstant';
...
console.log("openModalopenModalopenModal" , openModal);
  ...
  const { data: roomData } = useQuery({
    queryKey: ["get-roomData"],
    queryFn: () =>
      Apiservice.getAuth(`${API_ROOM_GET}?search=${""}`, token ?? ""),
    staleTime: 5 * 60 * 1000,
  });
  ...
  const columns: MRT_ColumnDef<MedicineData>[] = [
    ...
    {
      header: 'Room With Breakfast Price',
      accessorKey: 'withBreakfastPrice',
      enableSorting: true,
      size: 120,
      Cell: ({ cell }: { cell: { getValue: () => string | null } }) =>
        cell.getValue() || 'N/A',
