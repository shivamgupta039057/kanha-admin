...
import { API_CANCLE_HALL_BOOK, API_CANCLE_ROOM_BOOK, API_ROOM_ADD, API_ROOM_UPDATE } from '../../utils/APIConstant.ts';
...
const CancelBookingModel: React.FC<ModalProps> = ({ handleToggelModal, openModal, updateRow, handleClearRow }) => {
  ...
  const addRoomMutation = useMutation({
    mutationFn: async () => {
      ...
      return await Apiservice.postAuth(`${API_CANCLE_HALL_BOOK}/${updateRow._id}`, {}, token);
    },
    onSuccess: (res: any) => {
      if (res && res.data.status) {
        queryClient.invalidateQueries({ queryKey: ["get-hall-booking-view"] });
