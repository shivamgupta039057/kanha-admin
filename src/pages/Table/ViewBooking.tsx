...
import { API_HALL_VIEW_BANQUET, API_ROOM_DELETE, API_ROOM_GET, API_VIEW_ROOM } from '../../utils/APIConstant';
...
const HallBooking: React.FC = () => {
    ...
    const { data: roomData } = useQuery({
        queryKey: ["get-hall-booking-view"],
        queryFn: () =>
            Apiservice.getAuth(`${API_HALL_VIEW_BANQUET}/${param.hallId}`, token ?? ""),
        staleTime: 5 * 60 * 1000,
    });
    ...
    useEffect(() => {
        if (roomData) {
            setData(roomData?.data?.data?.bookings);
    ...
    return (
        <>
            <div className="flex justify-between items-start sm:items-center mb-6 gap-3 flex-col sm:flex-row">
                <Breadcrumb pageName="Banquet Booking" />
            </div>
