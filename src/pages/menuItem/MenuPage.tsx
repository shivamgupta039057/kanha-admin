...
import AddRoomsRegistration from './AddMenuModal';
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { API_BANQUET_DELETE, API_BANQUET_GET, API_DELETE_GALLERY, API_DELETE_MENU, API_GET_GALLERY, API_GET_MENU, API_ROOM_DELETE, API_ROOM_GET } from '../../utils/APIConstant';
...
const MenuPage: React.FC = () => {
  ...
  console.log("paramparamparamparamparam", param.categoryId);
console.log("openModalopenModalopenModal" , openModal);

  const { data: roomData } = useQuery({
    queryKey: ["get-menu-list" , param?.categoryId],
    queryFn: () =>
      Apiservice.getAuth(`${API_GET_MENU}/${param?.categoryId}`, token ?? ""),
    staleTime: 5 * 60 * 1000,
  });
  ...
  const deleteRoomMutation = useMutation({
    ...
    onSuccess: (res: any) => {
      if (res && res.data.status) {
        queryClient.invalidateQueries({ queryKey: ["get-menu-list" , param?.categoryId] });
