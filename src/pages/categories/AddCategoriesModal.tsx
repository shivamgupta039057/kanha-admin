...
import { getValidationSchema } from './CategoriesSchema.ts';
import { API_ADD_BANQUET, API_ADD_CATEGORY, API_ADD_GALLERY, API_BANQUET_UPDATE, API_ROOM_ADD, API_ROOM_UPDATE, API_UPDATE_CATEGORY, API_UPDATE_GALLERY } from '../../utils/APIConstant.ts';
...
const AddCategoriesModal: React.FC<ModalProps> = ({ handleToggelModal, openModal, updateRow, handleClearRow }) => {
  ...
  const addRoomMutation = useMutation({
    mutationFn: async (data: { name: string; description: string }) => {
      return await Apiservice.postAuth(`${API_ADD_CATEGORY}`, data, token);
    },
    onSuccess: (res: any) => {
  ...
  const editRoomMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string, data: { name: string; description: string } }) => {
      return await Apiservice.postAuth(`${API_UPDATE_CATEGORY}/${id}`, data, token);
    },
    onSuccess: (res: any) => {  
      if (res && res.data.status) {
